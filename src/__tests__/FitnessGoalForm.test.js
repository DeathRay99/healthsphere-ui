...
describe("FitnessGoalForm Component", () => {
  beforeEach(() => {
    ...
    window.alert = jest.fn(); // Reset alerts before each test
  });
  ...
  it("shows an alert when target date is before or equal to start date", () => {
    ...
    expect(window.alert).toHaveBeenCalledWith(
      "Target date must be greater than the start date."
    );
  });

  it("shows an alert when target weight is zero or negative", () => {
    render(
      <FitnessGoalForm
        userId={1}
        onSubmit={mockOnSubmit}
    ...
    expect(window.alert).toHaveBeenCalledWith(
      "Target weight cannot be zero or negative."
    );
  });

  it("shows an alert when body fat percentage is zero or negative", () => {
    render(
      <FitnessGoalForm
        userId={1}
        onSubmit={mockOnSubmit}
    ...
    fireEvent.change(screen.getByLabelText(/Target Body Fat/i), {
      target: { value: "-10" },
    });
    fireEvent.click(screen.getByRole("button", { name: /submit/i }));

    expect(window.alert).toHaveBeenCalledWith(
      "Body fat percentage cannot be zero or negative."
    );
  });
