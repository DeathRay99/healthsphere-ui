...
export default function EditConsultant() {
  ...
  const { id: consultantId } = useParams(); // Fetch the consultant ID dynamically from the URL
  const router = useRouter();

  // Fetch consultant details when the page loads
  useEffect(() => {
    if (consultantId) {
  ...
  // Fetch consultant details based on the ID
  const fetchConsultantDetails = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setFormData(data); // Populate form fields with consultant data
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push("/adminView/Consultant"); // Redirect back to the directory
      } else {
        const errorText = await response.text();
  ...
  if (!consultantId) {
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center text-red-500">Consultant ID is missing. Please check the URL.</div>
      </div>
    );
  }
  ...
  // Render the form for editing consultant details
  return (
