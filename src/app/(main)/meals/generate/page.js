...
function generateMeals() {
  ...
  const generateNewMeals = async () => {
    ...
    if (!canGenerate) {
      alert(
        `Please follow today's meal plan first! New meals will be available in ${timeUntilNextGeneration}.`
      );
      return;
    }
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="text-center mb-16">
        ...
        <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-10">
          Get a fresh, personalized meal plan every day based on your
          nutritional goals, preferences, and dietary restrictions —
          intelligently designed by AI.
        </p>

        {!canGenerate && lastGenerationDate && (
          <div className="bg-amber-50 border-l-4 border-amber-500 p-4 rounded-md mb-6 max-w-2xl mx-auto">
            <p className="text-amber-700 font-medium">
              Your next meal plan will be available in {timeUntilNextGeneration}
            </p>
            <p className="mt-2 text-gray-700">
              Stick to today's nutrition plan for optimal results!
            </p>
          </div>
        )}
        ...
        <button
          ...
          } text-white font-bold py-3 px-8 rounded-full shadow-lg hover:shadow-xl transform hover:-translate-y-1 transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed`}
        >
          {isLoading ? (
      ...
      {meals.length === 0 && !isLoading && (
        <div className="text-center mt-16 p-8 border-2 border-dashed border-gray-300 rounded-lg">
          <h3 className="text-xl font-medium text-gray-700 mb-2">
            No Meals Generated for Today Yet
          </h3>
          <p className="text-gray-500 mb-4">
            Click the button above to generate your personalized daily meal plan
          </p>
        </div>
      )}
    </div>
